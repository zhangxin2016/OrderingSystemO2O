		...
		<div id="tab_demo" class="HuiTab">
			...
			<div class="tabCon">
				<table
					...
					<thead>
						<tr class="text-c">
							<th width="80" class="hidden-xs1">商家名字</th>
							<th width="100" class="hidden-xs2">商家真实姓名</th>
							<th width="100" class="hidden-xs3">商家密码</th>
							<th width="100" class="hidden-xs4">商家电话</th>
							<th width="100" class="hidden-xs4">商家身份证号码</th>
							<th width="100" class="hidden-xs4">商家地址</th>
							<%--<th width="150">权限</th>--%>
							<th width="150">操作</th>
					...
					<tbody id="table_all">
						<c:forEach items="${userSellsList}" var="userSellsList">
							<tr class="text-c">

								<td class="hidden-xs1">${userSellsList.usname}</td>
								<td class="hidden-xs2">${userSellsList.ustruename}</td>
								<td class="hidden-xs3">${userSellsList.uspassword}</td>
								<td class="hidden-xs4">${userSellsList.usphone}</td>
								<td class="hidden-xs4">${userSellsList.usidcard}</td>
								<td class="hidden-xs4">${userSellsList.usaddress}</td>
								<%--<td>${arr.authority}</td>--%>
								<td class="td-manage"><a title="查看店铺信息"
									href="javascript:;"
									onclick="findStoresByUserSell('店铺信息','<%=basePath%>getStoresByUserSell.html?usid=${userSellsList.usid}','500','300')"
				...
				<td colspan="8">
					<!-------------------------------分页插件↓----------------------------------------------------------------->
					<form action="<%=basePath%>getAllUserSellBack.html" method="POST" name="spForm" id="spForm">
...
//复选框
	function checkAll() {
        ...
        function findStoresByUserSell(title,url,w,h){
