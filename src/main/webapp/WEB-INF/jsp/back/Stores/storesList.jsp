	...
	<div class="page-container">
		...
		<div id="tab_demo" class="HuiTab">
			...
			<div class="tabCon">
				<table
					...
					<thead>
						<tr class="text-c">
							...
							<th width="80" class="hidden-xs1">店铺地址</th>
							<th width="100" class="hidden-xs2">店铺名称</th>
							<th width="100" class="hidden-xs3">店铺图片</th>
							<th width="100" class="hidden-xs4">店铺收藏次数</th>
							<%--<th width="150">权限</th>--%>
							<th width="150">操作</th>
					...
					<tbody id="table_all">
						<c:forEach items="${stores}" var="Stores">
							<tr class="text-c">
								<td class="hidden-xs1">${Stores.staddress}</td>
								<td class="hidden-xs2">${Stores.stname}</td>
								<td class="hidden-xs3"><img src="/pic/${Stores.stdesc}" width="100px" height="60px"></td>
								<td class="hidden-xs4">${Stores.uscoll}</td>
								<%--<td>${arr.authority}</td>--%>
								<td class="td-manage"><a title="查看信息"
									href="javascript:;"
									onclick="findStores('店铺信息','<%=basePath%>getStoresByStid.html?stid=${Stores.stid }','800','300')"
				...
				<td colspan="8">
					<!-------------------------------分页插件↓----------------------------------------------------------------->
					<form action="<%=basePath%>getAllStores.html" method="POST" name="spForm" id="spForm">
...
/*管理员编辑*/
function findStores(title,url,w,h){
