	...
	<div class="page-container">
		...
		<div id="tab_demo" class="HuiTab">
			...
			<div class="tabCon">
				<table
					...
					<thead>
						<tr class="text-c">
							...
							<th width="100" class="hidden-xs4">菜品收藏次数</th>
							<th width="100" class="hidden-xs4">菜品销售次数</th>
							<%--<th width="150">权限</th>--%>
							<th width="150">操作</th>
					...
					<tbody id="table_all">
						<c:forEach items="${foods}" var="foods">
							<tr class="text-c">
								<td class="hidden-xs1">${foods.fname}</td>
								<td class="hidden-xs2">￥${foods.fprice}</td>
								<td class="hidden-xs3"><img src="/pic/${foods.fpic}" width="100px" height="60px"></td>
								<td class="hidden-xs4">${foods.fcollection}</td>
								<td class="hidden-xs4">${foods.fsalesvolume}</td>
								<%--<td>${arr.authority}</td>--%>
				...
				<td colspan="8">
					<!-------------------------------分页插件↓----------------------------------------------------------------->
					<form action="<%=basePath%>getAllFoodFront.html" method="POST" name="spForm" id="spForm">
