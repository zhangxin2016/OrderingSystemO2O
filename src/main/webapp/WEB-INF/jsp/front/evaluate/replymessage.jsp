            ...
            $("#checkcontent").click(function(){
                $.ajax({
                    ...
                    success:function(data){
                        if(data.result == 0){
                            layer.msg('您还没有输入内容呢', {icon: 2,time: 1000});
                            return;
                        }else if(data.result == 1){
                            $("#error").html("");
                            layer.msg('回复留言成功', {icon: 1,time: 1000});
                            $("#demoform-2").submit();
                        }else if(data.result == 2){
                            $("#error").html("");
                            layer.msg('包含敏感字，请重新输入', {icon: 2,time: 2000});
                            return;
                        }else{
                            $("#error").html("");
                        }
...
<article class="page-container" style="padding-top: 1%;width: 90%;margin-left: 5%;">
	<form action="<%=basePath %>addSellEvaluate.html" method="post" class="form form-horizontal" id="demoform-2" novalidate="novalidate" enctype="multipart/form-data">
		<input type="hidden" class="input-text" name="eid" value="${eid}" id="eid" >
		<legend class="col-xs-12" style="font-size: 16px;padding: 0px;margin-top: 10px;margin-bottom: 0px;">请输入回复内容:</legend>
		<input type="text" class="input-text" name="esellcontent" value="" id="ebuycontent" >
